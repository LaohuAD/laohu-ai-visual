...
trap 'rm -f "$fixture_valid" "$fixture_timeline" "$fixture_no_shot" "$fixture_missing_section" "$fixture_direct_negative" "$fixture_author_explanation" "$fixture_director_intent" "$fixture_contrastive_explanation" "$fixture_ambiguous_focus" "$fixture_detached_dialogue" "$fixture_weak_shot_header" "$fixture_weak_shot_body" "$fixture_portrait_reference" "$fixture_portrait_reference_alias" "$fixture_portrait_reference_plain"' EXIT
