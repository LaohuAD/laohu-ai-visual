...
```text
角色资产：
人物素体形象资产图：需要锁定人物身份时使用；左侧头部正面特写；右侧左45度、正面、背面全身三视图；属性说明区
人物服装资产图：仅在服装结构需要独立复用或承担视觉重点时使用；头部穿戴、上半身、下半身、足部四区总览；局部结构、材质、穿法和配色细节
人物妆造 / 最终造型资产图：仅在同一套造型跨独立片段复用或必须锁定最终外观时使用；可直接合并服装、发型与妆容，不默认引用服装资产；左侧头部正面特写；右侧左45度、正面、背面三视图；必要的造型说明；使用受控定妆摄影背景与光线，确保身份与造型可读
...
镜头任务表：
跨镜头一致性要求：
剧情状态映射：稳定身份资产 / 持续可见阶段的独立资产或文字状态 / 只属于当前镜头的瞬时表演状态
...
表情占用时长：
基础设定：
人物参考占位：角色名：【素体参考@B01 人物素体形象资产】；按需补充【服装参考@W01 人物服装资产】或【妆造 / 最终造型参考@M01 人物妆造设计资产】；没有对应资产时在提示词正文描述本镜服装、发型、妆容和一次性物件；音频按需使用【音色参考@V1 人物音频参考】
氛围与画质：
画面内容：
质量边界：
生成前风险：
