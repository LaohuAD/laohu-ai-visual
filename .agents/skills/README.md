...
跨包真正需要的专业方法，以真实文件保存在消费包内置方法/中，来源专业负责维护。语言方法与交付渲染脚本同样有完整副本；这是为了独立上传而保留必要内容，不是快捷方式，也不是执行时远程取文件。维护者改动公共方法后，在同一轮更新所有受影响副本并校验。**使用者上传前不用导出或拼包。**
...
`update_skills_readme_tree.py` 按当前磁盘刷新下面的“五包实际文件树”：保留已有说明文字，只增删真实存在的文件，不虚构未创建的方法，也不改变既有条目的先后。改动包内文件后运行它，再跑路径校验。
...
包内文件链接不能越出本包；跨包需要的方法由对方专业负责，本包写清负责人名与读取入口，不复制对方正文，也不留失效链接。真正随包携带的必需方法才进入`内置方法/`，由来源专业统一维护。任何一个包单独打开、其他包和共享目录缺席时，仍能完成本包职责内的任务；缺少的只能是本次作品输入，不能是专业方法。
