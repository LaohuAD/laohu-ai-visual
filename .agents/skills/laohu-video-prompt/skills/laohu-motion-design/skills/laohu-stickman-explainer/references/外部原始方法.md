...
1. Read `外部分镜模板.md` and produce Phase A in the user's language, with English VO and a reference translation.
...
4. Only after approval of the current Phase A, read `外部OmniFlash提示词合同.md` and produce Phase B.
5. Use `外部案例.md` only when a concrete end-to-end example would resolve ambiguity.
